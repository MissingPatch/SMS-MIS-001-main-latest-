...
if (isset($_POST['verify'])) {
    ...
    echo "OTP: $otp<br>";
    echo "Email: $email<br>";
    $check_otp = "SELECT * FROM mis_student_users WHERE email='$email' AND otp='$otp'";
    $result = $con->query($check_otp) or die($con->error);
    $row = $result->fetch_assoc();
    $total = $result->num_rows;

    if ($total > 0 ) {
        $update_query = "UPDATE mis_student_users SET otp = null WHERE unique_id = {$row['unique_id']}";
        ...
        header("Location: ../student/student_auth.php?error=");
        exit();
    }
}
?>
