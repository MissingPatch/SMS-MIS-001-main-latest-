...
<div class="dropdown" style="float: right; padding: 5px">
    <a href="#" onclick="return false;" role="button" data-toggle="dropdown" id="dropdownMenu1" data-target="#" style="float: left" aria-expanded="true">
        <i class="bx bxs-bell fs-4 mt-1 m-0"><span class="report-badge" style="color: white; font-size: 10px; background-color: red; border-radius: 30%; border: 1px solid white; padding: 1px 3px; z-index: -1;margin-left: -0.9px;">
        <?php printf($rowcount);?></span></i></i>
    </a>
...
<script>
    $(document).ready(function() {
        setInterval(function() {
            $.ajax({
                url: "get_notification_reports.php",
                success: function(count) {
                    $(".report-badge").text(count);
                }
            });
        }, 1000);
    });
